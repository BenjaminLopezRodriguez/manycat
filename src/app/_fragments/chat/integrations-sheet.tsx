...
function IntegrationsBody({
  ...
  if (panel === "grid") {
    return (
      <>
        <label className="relative block shrink-0 px-5 pb-3">
          <HugeiconsIcon
            icon={Search01Icon}
            size={16}
            className="text-muted-foreground pointer-events-none absolute top-1/2 left-8 -translate-y-1/2"
          />
          <Input
            value={query}
    ...
    <div className="flex min-h-0 flex-1 flex-col overflow-y-auto px-5 py-4">
      ...
      <form onSubmit={submitRequest} className="flex flex-col gap-4">
        ...
        <Button
          type="submit"
          className="w-full rounded-2xl"
          disabled={isPending || requestName.trim().length === 0}
        >
  ...
  if (isMobile) {
    return (
      <Drawer
        ...
        showSwipeHandle
      >
        <DrawerContent className="max-h-[90dvh] md:hidden">
          <DrawerHeader className="border-border/80 border-b px-5 pt-2 pb-4 text-left">
            <DrawerTitle className="font-heading text-lg tracking-tight">
              Integrations
            </DrawerTitle>
            <DrawerDescription>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        ...
        showCloseButton
      >
        <DialogHeader className="border-border/80 border-b px-5 py-5 pr-14 text-left">
